...
**写作要求**
- 语言风格：使用口语化的演讲语言，避免照抄 bullet points。把要点展开成完整句子，必要时用比喻、类比、设问增强感染力。
...
- 缩略图文件夹名为 `<文件名>_thumbnails`，在 Markdown 中引用图片时，请确保路径中没有空格
### 3.4 写入输出文件

用 Write 工具将最终演讲稿**覆盖写入**第二步生成的 `.md` 文件，完成替换。同时确保引用缩略图成功。
