...
POPPLER_URLS = {
    "windows": "https://ghfast.top/https://github.com/oschwartz10612/poppler-windows/releases/download/v24.08.0-0/Release-24.08.0-0.zip",
}
...
def save_content_to_markdown(slides_data, md_path):
    """将提取的内容保存为 Markdown 格式"""
    with open(md_path, 'w', encoding='utf-8') as f:
        for slide in slides_data:
            f.write(f"# 第 {slide['page']} 页\n\n")
            if slide['title']:
                f.write(f"## 标题\n{slide['title']}\n\n")
            if slide['body']:
                f.write(f"## 正文\n{slide['body']}\n\n")
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # 3. 提取 PPT 内容
        slides_data = extract_pdf_content(pdf_file)
        save_content_to_markdown(slides_data, output_path)
